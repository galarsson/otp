...
sendfile(Config) when is_list(Config) ->
	...
	true ->
	    ...
	    ?line true = Error =:= ebadf orelse Error =:= enoent orelse Error =:= eio,
...
sendfile_send_chunked(Host, File) ->
...
sendfile_send(Host, File) ->
	...
	{server, Port} ->
	    ?line {ok, Sock} = gen_tcp:connect(Host, Port,
					       [binary, {packet,0}]),
	    ?line test_server:format("sendfile_send open ~p", [File]),
	    ?line {ok, Fd} = file:open(File, [read, raw, binary]),
	    ?line {ok, Size} = file:sendfile(Fd, Sock, 0, Size),
	    ?line ok = file:close(Fd),
	    ?line ok = gen_tcp:close(Sock),
	    receive
		{ok, Bin} ->
